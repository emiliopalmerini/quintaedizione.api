...
func (r *PostgresRepository) List(ctx context.Context, filter shared.ListFilter) ([]maestrie.Maestria, int, error) {
	query := `SELECT * FROM maestrie WHERE 1=1`
	countQuery := `SELECT COUNT(*) FROM maestrie WHERE 1=1`
...
func (r *PostgresRepository) GetByID(ctx context.Context, id string) (*maestrie.Maestria, error) {
	query := `SELECT * FROM maestrie WHERE id = $1`

	var item maestrie.Maestria
	if err := r.db.GetContext(ctx, &item, query, id); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, fmt.Errorf("get maestrie by id: %w", err)
	}
