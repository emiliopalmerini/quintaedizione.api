...
func (r *PostgresRepository) List(ctx context.Context, filter shared.ListFilter) ([]talenti.Talento, int, error) {
	query := `SELECT * FROM talenti WHERE 1=1`
	countQuery := `SELECT COUNT(*) FROM talenti WHERE 1=1`
...
func (r *PostgresRepository) GetByID(ctx context.Context, id string) (*talenti.Talento, error) {
	query := `SELECT * FROM talenti WHERE id = $1`

	var item talenti.Talento
	if err := r.db.GetContext(ctx, &item, query, id); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, fmt.Errorf("get talenti by id: %w", err)
	}
