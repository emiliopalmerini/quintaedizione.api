...
func (r *PostgresRepository) List(ctx context.Context, filter shared.ListFilter) ([]background.Background, int, error) {
	query := `SELECT * FROM background WHERE 1=1`
	countQuery := `SELECT COUNT(*) FROM background WHERE 1=1`
...
func (r *PostgresRepository) GetByID(ctx context.Context, id string) (*background.Background, error) {
	...
	if err := r.db.GetContext(ctx, &item, query, id); err != nil {
		...
		return nil, fmt.Errorf("get background by id: %w", err)
	}
