...
func (r *PostgresRepository) List(ctx context.Context, filter shared.ListFilter) ([]condizioni.Condizione, int, error) {
	query := `SELECT * FROM condizioni WHERE 1=1`
	countQuery := `SELECT COUNT(*) FROM condizioni WHERE 1=1`
	...
	var items []condizioni.Condizione
...
func (r *PostgresRepository) GetByID(ctx context.Context, id string) (*condizioni.Condizione, error) {
	query := `SELECT * FROM condizioni WHERE id = $1`

	var item condizioni.Condizione
	if err := r.db.GetContext(ctx, &item, query, id); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, fmt.Errorf("get condizioni by id: %w", err)
	}
