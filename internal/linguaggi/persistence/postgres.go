...
func (r *PostgresRepository) List(ctx context.Context, filter shared.ListFilter) ([]linguaggi.Linguaggio, int, error) {
	query := `SELECT * FROM linguaggi WHERE 1=1`
	countQuery := `SELECT COUNT(*) FROM linguaggi WHERE 1=1`
	...
	var items []linguaggi.Linguaggio
...
func (r *PostgresRepository) GetByID(ctx context.Context, id string) (*linguaggi.Linguaggio, error) {
	query := `SELECT * FROM linguaggi WHERE id = $1`

	var item linguaggi.Linguaggio
	if err := r.db.GetContext(ctx, &item, query, id); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, fmt.Errorf("get linguaggi by id: %w", err)
	}
