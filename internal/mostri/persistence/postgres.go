...
func (r *PostgresRepository) List(ctx context.Context, filter shared.ListFilter) ([]mostri.Mostro, int, error) {
	query := `SELECT * FROM mostri WHERE 1=1`
	countQuery := `SELECT COUNT(*) FROM mostri WHERE 1=1`
...
func (r *PostgresRepository) GetByID(ctx context.Context, id string) (*mostri.Mostro, error) {
	query := `SELECT * FROM mostri WHERE id = $1`

	var item mostri.Mostro
	if err := r.db.GetContext(ctx, &item, query, id); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, fmt.Errorf("get mostri by id: %w", err)
	}
