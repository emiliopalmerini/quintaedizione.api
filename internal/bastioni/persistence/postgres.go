...
func (r *PostgresRepository) List(ctx context.Context, filter shared.ListFilter) ([]bastioni.Bastione, int, error) {
	query := `SELECT * FROM bastioni WHERE 1=1`
	countQuery := `SELECT COUNT(*) FROM bastioni WHERE 1=1`
...
func (r *PostgresRepository) GetByID(ctx context.Context, id string) (*bastioni.Bastione, error) {
	query := `SELECT * FROM bastioni WHERE id = $1`

	var item bastioni.Bastione
	if err := r.db.GetContext(ctx, &item, query, id); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, fmt.Errorf("get bastioni by id: %w", err)
	}
