...
func (r *PostgresRepository) List(ctx context.Context, filter shared.ListFilter) ([]divinita.Divinita, int, error) {
	query := `SELECT * FROM divinita WHERE 1=1`
	countQuery := `SELECT COUNT(*) FROM divinita WHERE 1=1`
...
func (r *PostgresRepository) GetByID(ctx context.Context, id string) (*divinita.Divinita, error) {
	query := `SELECT * FROM divinita WHERE id = $1`

	var item divinita.Divinita
	if err := r.db.GetContext(ctx, &item, query, id); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, fmt.Errorf("get divinita by id: %w", err)
	}
