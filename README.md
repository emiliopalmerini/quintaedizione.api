...
Gli endpoint `/v1/*` richiedono autenticazione tramite API Key (se configurata).

```bash
curl -H "X-API-Key: your-secret-key" http://localhost:8080/v1/classi
```

Per disabilitare l'autenticazione, lasciare `API_KEY` vuoto in `.env`.

## Rate Limiting

L'API limita le richieste per IP (default: 60 richieste/minuto).
Configurabile tramite `RATE_LIMIT_RPM` e `RATE_LIMIT_ENABLED`.

## CORS

CORS abilitato di default per tutte le origini (`*`).
Configurabile tramite `CORS_ALLOWED_ORIGINS` e `CORS_ALLOWED_METHODS`.
