...
func (r *PostgresRepository) List(ctx context.Context, filter shared.ListFilter) ([]oggetti.Oggetto, int, error) {
	query := `SELECT * FROM oggetti WHERE 1=1`
	countQuery := `SELECT COUNT(*) FROM oggetti WHERE 1=1`
...
func (r *PostgresRepository) GetByID(ctx context.Context, id string) (*oggetti.Oggetto, error) {
	query := `SELECT * FROM oggetti WHERE id = $1`

	var item oggetti.Oggetto
	if err := r.db.GetContext(ctx, &item, query, id); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, fmt.Errorf("get oggetti by id: %w", err)
	}
